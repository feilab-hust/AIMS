    ...
    if path:
        for p in Path(path).iterdir():
            ...
            if name.strip().lower() != 'young':  # and 'manually selected' in p.name:
                ...
                continue
                # raise ValueError('only Old (various methods) and Young are supported')
            if not is_mask:
                if label != '0':
                ...
                if label != '0':
                    path_load_mask(p / '3D-WF (single)', list_old, label)
                else:
                    path_load_mask(p / '3D-WF (single)', list_young, label)
        ...
        for dir_name, AC_list_test in AC_list_test_groups.items():
            ...
            with torch.no_grad():
                for test_p_dict in tqdm(AC_list_test,
                    ...
                    if hp.data.dual_class and predict_class != 0:
                        predict_class = class2label[f'{hp.data.sense_class}']
